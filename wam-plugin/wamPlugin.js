...
class WamPlugin extends HTMLElement {
    constructor() {
        ...
        //get the src attribute. wam-plugin MUST BE IN THE ROOT DIR above plugin folders
    ...
    loadPlugin = async (audioContext,hostGroupId) => {
        ...
        // Load the GUI if needed (ie. if the option noGui was set to true)
    ...
    /**  First parameter is the audio node of the destination
     * Second one is the instance of the destination (for checking its descriptor in order
     * to check if it has midi and audio I/O)
     */
    connectPlugin = (destAudioNode,destInstance) => {
        const audioNode = this.instance.audioNode;

        if(destInstance)
            console.log("WAM-PLUGIN : connecting " + this.instance.descriptor.name + " to " + destInstance.descriptor.name) 
        else 
            console.log("WAM-PLUGIN : connecting " + this.instance.descriptor.name + " to destination");

        // Let's verify current plugin
        // properties such as hasMidiInput, hasMidiOutput
        // hasAudioInput, and hasAudioOutput
        // and do the output connections accordingly
        if(this.instance.descriptor.hasAudioOutput) {
            console.log(this.instance.descriptor.name + " has audioOutput");
            if((destAudioNode === this.audioContext.destination) || destInstance.descriptor.hasAudioInput) {
                if(destAudioNode === this.audioContext.destination)
                    console.log(" connecting to destination")
                else
                console.log(" connecting audio to " + destInstance.descriptor.name)
                audioNode.connect(destAudioNode);
            }
        }

        if((this.instance.descriptor.hasMidiOutput) && (destInstance.descriptor.hasMidiInput)) {
            console.log(this.instance.descriptor.name + " has midiOutput and " + destInstance.descriptor.name + " has midiInput")
            audioNode.connectEvents(destInstance.instanceId);
        }
    }
