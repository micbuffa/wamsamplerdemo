...
class WamHost extends HTMLElement {
    constructor() {
        ...
        this.mount = this.root.querySelector('.wam-host');
        this.mountPlug = this.root.querySelector('.wam-plugins');
        this.audioInput = this.root.querySelector('#selectAudioInput');
        this.firstAudioNode;
        // get the toggleLiveInput button
        const liveInputButton = this.root.querySelector('#toggleLiveInput');
        liveInputButton.onclick = this.toggleLiveInput.bind(this, this.root);
        ...
        if (navigator.requestMIDIAccess) {
            navigator.requestMIDIAccess().then((midiAccess) => {
                ...
                const handleMidiMessage = (e) => {
                    ...
                    // schedule this event to all plugins that
                    // have a midiInput
                    for(let i = 0; i < this.plugins.length; i++) {
                        // current Plugin
                        const plugin = this.plugins[i];
                        const pluginInstance = plugin.instance;
                        const pluginAudioNode = pluginInstance.audioNode;
            
                        // connect keyboard to the plugin only if it has midiInput
                        if(pluginInstance.descriptor.hasMidiInput) {
                            pluginAudioNode.scheduleEvents({ type: 'wam-midi', time: pluginAudioNode.context.currentTime, data: { bytes: e.data } });
                            //this.audioContext.resume();
                            //this.firstAudioNode.scheduleEvents({ type: 'wam-midi', time: this.firstAudioNode.context.currentTime, data: { bytes: e.data } });
                        }
                    }

                    // make the virtual keyboard listen to external keyboards
                    this.virtualKeyboard.audioNode.scheduleEvents({ type: 'wam-midi', time: this.virtualKeyboard.audioNode.context.currentTime, data: { bytes: e.data } });
/*
    ...
    connectPlugins = async () => {
        for(let i = 0; i < this.plugins.length; i++) {
            ...
            let destAudioNode, destInstance;

            if(i === this.plugins.length - 1) {
                //last plugin of the chain
                destAudioNode = this.audioContext.destination;
            } else {
                destInstance = this.plugins[i+1].instance;
                destAudioNode = destInstance.audioNode
            }
            // destInstance is undefined if destAudioNode = destination
            plugin.connectPlugin(destAudioNode, destInstance);
        }
    };

    // connect the keyboard to the first plugin
    connectKeyboard = (keyboardAudioNode) => {
        for(let i = 0; i < this.plugins.length; i++) {
            // current Plugin
            const plugin = this.plugins[i];
            const pluginInstance = plugin.instance;
            ...
            // connect keyboard to the plugin only if it has midiInput
            if(pluginInstance.descriptor.hasMidiInput) {
                keyboardAudioNode.connectEvents(pluginAudioNode.instanceId);
            }
    ...
    loadInterface = async () => {
        ...
        const loadInstrumentInterface =  async () => {
            ...
            this.connectKeyboard(instanceKeyboard.audioNode);
        ...
        // Check if there is a plugin with midiInput
        // in that case, load the instrument host GUI elements (midi input menu, midi virtual keyboard)
        let pluginWithMidiInputInChain = this.checkPluginWithMidiInputInChain();
        // Check is there is a plugin with audioInput in chain
        let pluginWithAudioInputInChain = this.checkPluginWithAudioInputInChain();
     
        // check if the plugin is an instrument or an effect and load the corresponding interface
        if(pluginWithMidiInputInChain) {
            this.virtualKeyboard = await loadInstrumentInterface();
        }
        
        
        if(pluginWithAudioInputInChain) {
            loadEffectInterface(pluginWithAudioInputInChain);
        }
        
    }

    checkPluginWithMidiInputInChain() {
        for(let i = 0; i < this.plugins.length; i++) {
            // current Plugin
            const plugin = this.plugins[i];
            const pluginInstance = plugin.instance;
    ...
    checkPluginWithAudioInputInChain() {
        for(let i = 0; i < this.plugins.length; i++) {
            // current Plugin
            const plugin = this.plugins[i];
            const pluginInstance = plugin.instance;
            ...
            // connect keyboard to the plugin only if it has midiInput
            if(pluginInstance.descriptor.hasAudioInput)
